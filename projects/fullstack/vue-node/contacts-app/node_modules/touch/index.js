...
const validOpts = (options, path, fd) => {
  ...
  if (!options.atime && !options.mtime) options.atime = options.mtime = now;
  else {
...
class Touch extends EE {
  ...
  onopen(er, fd) {
    if (er) {
      if (er.code === "EISDIR") this.onopen(null, null);
      else if (er.code === "ENOENT" && this.nocreate) this.emit("done");
      else this.emit("error", er);
    } else {
      this.fd = fd;
      if (this.ref) this.statref();
      else if (!this.atime || !this.mtime) this.fstat();
      else this.futimes();
    }
  }
  ...
  onstatref(st) {
    ...
    if (!this.atime || !this.mtime) this.fstat();
    else this.futimes();
  }
