...
function nodemon(settings) {
  ...
  config.load(settings, function(config) {
    if (
      !config.options.dump &&
      !config.options.execOptions.script &&
    ...
    if (config.options.restartable) {
      utils.log.info(
        "to restart at any time, enter `" + config.options.restartable + "`"
      );
    }

    if (!config.required) {
      const restartSignal =
        config.options.signal === "SIGUSR2" ? "SIGHUP" : "SIGUSR2";
      process.on(restartSignal, nodemon.restart);
      utils.bus.on("error", () => {
        utils.log.fail(new Error().stack);
      });
      utils.log.detail(
        (config.options.restartable ? "or " : "") +
          "send " +
          restartSignal +
          " to " +
          process.pid +
    ...
    utils.log.info(
      "watching extensions: " + (config.options.execOptions.ext || "(all)")
    );
    ...
    if (config.options.events && Object.keys(config.options.events).length) {
      Object.keys(config.options.events).forEach(function(key) {
        utils.log.detail(
          "bind " + key + " -> `" + config.options.events[key] + "`"
        );
