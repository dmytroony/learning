      ...
      "Test inflector method": {
        ...
        camelize: {
          ...
          "path with first letter lower": function(topic) {
            return assert.equal(
              topic.camelize("bullet_record/errors", false),
              "bulletRecord.Errors"
            );
          }
        },
        underscore: {
          ...
          "from dasherize": function(topic) {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              i = _ref[_i];
              _results.push(
                assert.equal(topic.underscore(topic.dasherize(i)), i)
              );
            }
            return _results;
          }
        ...
        demodulize: {
          "module name": function(topic) {
            return assert.equal(
              topic.demodulize("BulletRecord.CoreExtensions.Inflections"),
              "Inflections"
            );
          },
        ...
        foreign_key: {
          normal: function(topic) {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              i = _ref[_i];
              _results.push(
                assert.equal(topic.foreign_key(i, false), words[i])
              );
            }
            return _results;
          },
      ...
      "Test inflector inflection methods": {
        ...
        pluralize: {
          ...
          cases: function(topic) {
            ...
            for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
              ...
              _results.push(
                assert.equal(
                  topic.pluralize(util.string.capitalize(i)),
                  util.string.capitalize(words[i])
                )
              );
            }
            return _results;
          ...
          "cases plural": function(topic) {
            ...
            for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
              ...
              _results.push(
                assert.equal(
                  topic.pluralize(util.string.capitalize(words[i])),
                  util.string.capitalize(words[i])
                )
              );
            }
        ...
        singuralize: {
          ...
          cases: function(topic) {
            ...
            for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
              ...
              _results.push(
                assert.equal(
                  topic.singularize(util.string.capitalize(words[i])),
                  util.string.capitalize(i)
                )
              );
            }
            return _results;
        ...
        uncountablility: {
          normal: function(topic) {
            ...
            for (_k = 0, _len3 = words.length; _k < _len3; _k++) {
              i = words[_k];
              _results.push(
                assert.equal(topic.singularize(i), topic.pluralize(i))
              );
            }
            return _results;
          },
          greedy: function(topic) {
            ...
            assert.equal(
              topic.pluralize(uncountable_word),
              topic.singularize(uncountable_word)
            );
            assert.equal(topic.singularize(countable_word), "sponsor");
            assert.equal(topic.pluralize(countable_word), "sponsors");
            return assert.equal(
              topic.singularize(topic.pluralize(countable_word)),
              "sponsor"
            );
          }
        },
        ...
        titleize: {
          ...
          "with hyphens": function(topic) {
            return assert.equal(
              topic.titleize("x-men: the last stand"),
              "X Men: The Last Stand"
            );
          },
          "with ampersands": function(topic) {
            return assert.equal(
              topic.titleize("garfunkel & oates"),
              "Garfunkel & Oates"
            );
          }
        },
        ...
        classify: {
          underscore: function(topic) {
            ...
            for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
              i = _ref2[_j];
              _results.push(
                assert.equal(topic.classify("table_prefix." + words[i]), i)
              );
            }
            return _results;
          },
