...
test("parse()", function(t) {
    ...
    t.test("parses a string with an alternative string delimiter", function(
        st
    ) {
    ...
    t.test("parses a string with an alternative RegExp delimiter", function(
        st
    ) {
        st.deepEqual(qs.parse("a=b; c=d", { delimiter: /[;,] */ }), {
            a: "b",
    ...
    t.test("parses string with comma as array divider", function(st) {
        ...
        st.deepEqual(qs.parse("foo", { comma: true }), { foo: "" });
        st.deepEqual(
            qs.parse("foo", { comma: true, strictNullHandling: true }),
            { foo: null }
        );
        st.end();
    });
    ...
    t.test("parses null objects correctly", { skip: !Object.create }, function(
        st
    ) {
        ...
        st.deepEqual(
            qs.parse("%8c%a7=%91%e5%8d%e3%95%7b", {
                decoder: function(str) {
                    ...
                    return String(
                        iconv.decode(SaferBuffer.from(result), "shift_jis")
                    );
                }
            }),
            { 県: "大阪府" }
    ...
    t.test("should ignore an utf8 sentinel with an unknown value", function(
        st
    ) {
        st.deepEqual(
            qs.parse(
