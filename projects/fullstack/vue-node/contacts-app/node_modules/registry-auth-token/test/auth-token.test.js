...
describe("auth-token", function() {
  ...
  it("should return undefined if no auth token is given for registry", function(done) {
    fs.writeFile(npmRcPath, "registry=http://registry.npmjs.eu/", function(
      err
    ) {
      var getAuthToken = requireUncached("../index");
      assert(!err, err);
  ...
  describe("legacy auth token", function() {
    it("should return auth token if it is defined in the legacy way via the `_auth` key", function(done) {
      ...
      fs.writeFile(npmRcPath, content, function(err) {
        var getAuthToken = requireUncached("../index");
        assert(!err, err);
        assert.deepStrictEqual(getAuthToken(), {
          token: "foobar",
          type: "Basic"
        });
        done();
  ...
  describe("bearer token", function() {
    ...
    it("should use npmrc passed in", function(done) {
      ...
      fs.writeFile(npmRcPath, content, function(err) {
        ...
        assert.deepStrictEqual(getAuthToken({ npmrc: npmrc }), {
          token: "qar",
          type: "Bearer"
        });
        done();
    ...
    it("should fetch for the registry given (if defined)", function(done) {
      ...
      fs.writeFile(npmRcPath, content, function(err) {
        var getAuthToken = requireUncached("../index");
        assert(!err, err);
        assert.deepStrictEqual(getAuthToken("//registry.blah.foo"), {
          token: "whatev",
          type: "Bearer"
        });
        done();
    ...
    it("recursively finds registries for deep url if option is set", function(done, undef) {
      ...
      fs.writeFile(npmRcPath, content, function(err) {
        ...
        assert.deepStrictEqual(
          getAuthToken("https://registry.blah.edu/foo/bar/baz", opts),
          { token: "recurseNoLevel", type: "Bearer" }
        );
        assert.deepStrictEqual(
          getAuthToken("https://registry.blah.org/foo/bar/baz", opts),
          { token: "recurseExactlyOneLevel", type: "Bearer" }
        );
        assert.deepStrictEqual(
          getAuthToken("https://registry.blah.com/foo/bar/baz", opts),
          { token: "whatev", type: "Bearer" }
        );
        assert.deepStrictEqual(
          getAuthToken("http://registry.blah.eu/what/ever", opts),
    ...
    it("should try both with and without trailing slash", function(done) {
      fs.writeFile(npmRcPath, "//registry.blah.com:_authToken=whatev", function(
        err
      ) {
        var getAuthToken = requireUncached("../index");
        assert(!err, err);
        assert.deepStrictEqual(getAuthToken("https://registry.blah.com"), {
          token: "whatev",
          type: "Bearer"
        });
        done();
    ...
    it("should prefer bearer token over basic token", function(done) {
      ...
      fs.writeFile(npmRcPath, content, function(err) {
        var getAuthToken = requireUncached("../index");
        assert(!err, err);
        assert.deepStrictEqual(getAuthToken("//registry.foobar.eu"), {
          token: "bearerToken",
          type: "Bearer"
        });
        done();
