...
module.exports = function() {
  ...
  return (
    getRegistryAuthInfo(checkUrl, options) || getLegacyAuthInfo(options.npmrc)
  );
};
...
function getAuthInfoForUrl(regUrl, npmrc) {
  ...
  var bearerAuth = getBearerToken(
    npmrc[regUrl + tokenKey] || npmrc[regUrl + "/" + tokenKey]
  );
  if (bearerAuth) {
    return bearerAuth;
  }
  ...
  var password =
    npmrc[regUrl + passwordKey] || npmrc[regUrl + "/" + passwordKey];
  var basicAuth = getTokenForUsernameAndPassword(username, password);
  if (basicAuth) {
    return basicAuth;
  }

  var basicAuthWithToken = getLegacyAuthToken(
    npmrc[regUrl + legacyTokenKey] || npmrc[regUrl + "/" + legacyTokenKey]
  );
